...
- `locations` — list Azure regions with display name, geography, and paired
  region, fetched live from ARM so the list reflects what the subscription
  can use; unpaired (zone-redundant-only) regions show `-`. Supports the
  same `--name` pattern semantics as `search` (substring, `foo*`, `*foo`,
  `f*o`) and `--output json`
...
- `search` across multiple resource types (including bare `akvutil search`)
  failed with `ARM request failed (400 Bad Request)`: Azure Resource Graph
  rejects the leading `union (query), (query)` form. Multi-type queries now
  pipe the first branch into `| union (...)`, which Resource Graph accepts
